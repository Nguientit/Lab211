...
//        }
        int numBills = Validator.checkInputInt("input number of bill:", 1);
        int[] bills = new int[numBills];
        for (int i = 0; i < numBills; i++) {
            bills[i] = Validator.checkInputInt("input value of bill" + (i + 1) + ": ", 0);
        }

        int walletAmount = Validator.checkInputInt("input wallet amount: ", 0);

        Person person = new Person(walletAmount);

        int totalBills = person.calcTotal(bills);
        System.out.println("\nthis is total of bill: " + totalBills);

        boolean canPay = person.getWallet().payMoney(totalBills);

        if (canPay) {
            System.out.println("You can buy it.");
        } else {
            System.out.println("You can'1 buy it.");
        }
    }
}
